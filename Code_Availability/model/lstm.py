...
class LSTMModel(torch.nn.Module):
    def __init__(self,
        ...
        # self.med_encoder = torch.nn.LSTM(med_embedding_size, med_hidden_size, batch_first=True, bidirectional=bidirectional)  # , dropout=0.5)
        ...
        # )
        self.lstm2hidden_ipw = torch.nn.Sequential(
            torch.nn.Linear(diag_hidden_size + 4, hidden_size), # 3 dim for age and 1 dim for gender
            torch.nn.ReLU(),
        )
    ...
    def diag_encode(self, inputs):
        ...
        att_normalized = self.softmax_masked(att_enc, inputs_mask)  # temporal attention, not
    ...
    def forward(self, confounder):
        diag_inputs, sexes, ages = confounder
        ...
        original = torch.cat((diag_original,
